...
def func_get_outcome(df_oi, empi_to_death_date_dic, empi_to_date_oi_dic, get_biopsy_based_feature = False, empi_to_last_psa_check_date_dic_biopsy_based = None, clean_mode = False):
...
def get_psa_prior_to_rp(df_lab_total, empi_to_date_oi_dic, empis_oi):#, empis_oi, get_biopsy_based_feature = False):
...
#                             breakpoint()
	...
	df_outcome = pd.DataFrame([], index = empi_recurrence_psa_date_dic.keys(), columns = ['rp_date', 'first_post_op_psa', 'first_post_op_psa_date', 'first_post_op_psa_date_minus_rp_date_in_days', 
																							'first_post_op_psa_valid', 'post_op_psa_trajectory', 'post_op_psa_trajectory_dates', 'hormone_post_op', 
																							'bcr_date', 'bcr_date_minus_rp_date_in_days', 'pre_op_psa_trajectory_10_year_window', 'pre_op_psa_trajectory_dates']) # bcr_rp is set to NaN if they first_post_op_psa_date_minus_rp_date_in_days > 6 month																		  
	empi_to_last_psa_check_date_dic = {}
	# if get_biopsy_based_feature:
	# 	df_outcome['rp_failure'] = None
	lab_val_thresh_validity = 0.1
	for empi, list_oi in empi_recurrence_psa_date_dic.items():
		first_post_op_recorded = 0; record_bcr_event = 0
		RP_date = empi_to_date_oi_dic[empi]    
		psa_levels_before_rp = []; psa_levels_before_rp_dates = []
...
def main(data_comb, load_processed = False, get_biopsy_based_feature = False):
	...
	if load_processed:
		...
		if get_biopsy_based_feature:
			...
			df_labs_processed = get_psa_prior_to_rp(df_labs, empi_to_date_oi_dic, empis_oi)#, get_biopsy_based_feature = get_biopsy_based_feature)
			# breakpoint()
	...
	if get_outcome:
		...
		if get_biopsy_based_feature:
			...
			df_outcome_oi = func_get_outcome(df_pathology_biopsy_final, empi_to_death_date_dic, empi_to_date_oi_dic, get_biopsy_based_feature = get_biopsy_based_feature, empi_to_last_psa_check_date_dic_biopsy_based = empi_to_last_psa_check_date_dic_biopsy_based, clean_mode = clean_mode)
			if clean_mode:
