...
4. the number of positive cores

Things to consider for the extraction :
- The first mention of gleason before any biopsies in each report provides overall grade (3) along with primary grade (1) and secondary grade (2).
Then, map primary and secondary grades to overall grade group based on https://www.cancerresearchuk.org/about-cancer/prostate-cancer/stages/grades
- A biopsy info is followed after directional words like "right" base, "left" base, "right" mid
- The number of positive nodes corresponds to the number of times word "gleason" mentioned after the overall grade information
"""
...
def main():
	...
	if load_prcossed:
		...
		# df_pathology = pd.read_csv('../data/Pat.csv', sep="|", low_memory=False)
		...
		# df_pathology_with_rp = df_pathology[((df_pathology['Report_Text'].str.contains('prostatect|prostate radical resection|prostate, radical resection', case=False)) 
		# 									| (df_pathology['Report_Text'].str.contains('prostat', case=False) & df_pathology['Report_Text'].str.contains('margin', case=False))) 
		# 									& (~df_pathology['Report_Text'].str.contains('ureter|cystect|bone|Specimen Type: Prostate needle biopsy', case=False))]
		...
		num_pos_cores_list = []; primary_grade_list = []; secondary_grade_list = []; overall_grade_list = []; radical_pros = [];
		...
		pt_stage_false_terms = ['acceptor', 'transcripts']
		query_word = []
		for text_oi, report_number in tqdm(zip(df_pathology_with_rp.Report_Text.values, df_pathology_with_rp.Report_Number.values), total = len(df_pathology_with_rp), desc = 'extracing info...'):
			num_pos_cores = 0; radical_pros_ind = 0; gleason_reported = 0; primary_grade = None; secondary_grade = None; n1_val = None
		#   all patient should have received rp
			...
			# if 'n1' in text_oi:
			# 	idx_n1 = text_oi.find('n1')
			# 	print(text_oi)
			# 	print(text_oi[idx_n1-20:idx_n1+20])
			# 	breakpoint()

			# if 'right base' in text_oi and 'right apex' in text_oi and 'left base' in text_oi:
			# 	print(text_oi)
			# 	breakpoint()

			# if sentence_to_check.lower() in text_oi:
			# 	print(text_oi)
			# 	breakpoint()
			...
			# check for hard-coded rules (margin)
			...
			for val_margin_harcoded_neg in hard_coded_margin_negative:
				if val_margin_harcoded_neg in text_oi:
					if margin_ind:
						# print('(Margin) presence of both positive and negative sentences in Report : ', report_number)
						# print(val_margin_harcoded_neg)
						# print(val_margin_harcoded_pos)
						# print('\n')
						margin_list.pop()
			...
			for idx, val in enumerate(text_oi_list):
				...
				# get stage info
				if not tumor_stage_ind: # this ensures that the algorithm doesn't check the report again once tumor stage has been extracted
					if 'stage' in val or 'pt' in val and 'pten' not in val and 'ptpn' not in val and val not in pt_stage_false_terms:
						if 'clinical' not in text_oi_list[idx - 1]: # exclude clinical stage
							if 'n1' in ' '.join(text_oi_list[idx:idx+5]):
								n1_val = 1
								# breakpoint()
							# if 'stage' is in the report, look into the next 10 words
							for val_after in text_oi_list[idx:idx+10]:
								...
								if 't' in val_filtered: # if t is in the word
									digit_detcted = 0
									for idx_char, char in enumerate(val_filtered): # check if number follows after t 
										if 't' == char:
			...
			#                             remove punctuation and grab 2 chars after t to get stage
										# if val_filtered[idx_char:idx_char+3] == 't0':
										# 	print(text_oi_list[idx - 10:idx+10])
										# 	print(val_after)
										# 	print(val_filtered)
										# 	breakpoint()
										tumor_stage_list.append(val_filtered[idx_char:idx_char+3])
										if val_filtered[idx_char:idx_char+3] in query_word:
											print(text_oi_list[idx-5:idx+15])
											breakpoint()
										tumor_stage_ind = 1
						...
						tumor_stage_ind = 1
						if val in query_word:
							print(text_oi_list[idx-5:idx+15])
							breakpoint()
						# pass
						# print(val)
						# print(text_oi_list[idx:idx+5])
						# breakpoint()
					# if tumor_stage_list[-1] == 't0':
					# 	print(text_oi_list[idx-5:idx+15])
					# 	breakpoint()
		...
		#                 breakpoint()
				...
				# we report overall grade 
				if 'gleason' in val and radical_pros_ind and not gleason_reported:# not found_directional_word: 
					...
					for idx_char, char in enumerate(raw_gleason_text):
						...
						# else:
							...
							# breakpoint()
			# map primary and secondary grades to overall grade group
			# based on https://www.cancerresearchuk.org/about-cancer/prostate-cancer/stages/grades
			# overall_grade_group = None
			# if primary_grade == 3:
			# 	if secondary_grade == 3:
			# 		overall_grade_group = 1
			# 	elif secondary_grade == 4:
			...
			if primary_grade is not None and secondary_grade is not None:
				...
				overall_grade_group = None
			# update info
			if num_pos_cores == 0: # if no postiive nodes identified
				num_pos_cores_list.append(None)
			else:
				num_pos_cores_list.append(num_pos_cores)
	...
	"""
	# df_pathology_with_rp_oi = df_pathology_with_rp.copy()#.loc[df_pathology_with_rp.radical_prostatectomy == 1]
	...
	while continue_:
		...
		# sample_rp = input('Sample only from patients with radical prostatectomy? (Y/N) : ') == 'Y'
